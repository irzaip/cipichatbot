...
fileyaml = 'translate.yml'
fileinput = 'input.txt'
filestopword = 'stopword-id.txt'

rel_dir = '/data/translate/'
...
def get_entity(sentence, entname=None):
    ...
    #special token
    wordtoken.remove('indonesia')
    wordtoken.remove('inggris')
...
def process():
    global entdic
    kata=entdic['entity'][0]['value']
    print("memprosess terjemahan:", kata)

    try:
        import webbrowser
        url = 'https://translate.google.com/m/translate?hl=id&q='+str(kata)
        webbrowser.open(url)
    except:
        pass
