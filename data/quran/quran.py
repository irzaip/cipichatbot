...
def get_entity(sentence, entname=None):
    ...
    for i in entdic['entity']:
        if i['value']=='None':
            ...
            surat=entdic['entity'][0]['value']
            ayat=entdic['entity'][1]['value']
            command = "Membuka surat %s, ayat %s" % (surat, ayat )
...
def process():
    ...
    surat=entdic['entity'][0]['value']
    ayat=entdic['entity'][1]['value']
    print("memprosess quran:", surat, ayat)
    command="https://www.quran.com/%s/%s" % (surat,ayat)
